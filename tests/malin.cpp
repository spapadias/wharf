...
TEST_F(MalinTest, DEV)
{
    ...
    std::vector<std::pair<Edge*, uintV>> stream; std::vector<std::vector<Edge>> temp;

    std::ifstream file("data/wiki-graph"); int iter = 0;
    ...
    dygrl::Malin malin = dygrl::Malin(total_vertices, total_edges);
    malin.generate_initial_random_walks();
...
//    std::cout << stream.size() << std::endl;
    ...
    for(int i = 0; i < total_vertices * config::walks_per_vertex; i++)
    {
        std::cout << malin.walk(i) << std::endl;
    }
}
